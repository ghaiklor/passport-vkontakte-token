...
var VKontakteTokenStrategy = require('../');
var fakeProfile = JSON.stringify(require('./fixtures/profile.json'));

describe('VKontakteTokenStrategy:init', function () {
  it('Should properly export Strategy constructor', function () {
    assert.equal(typeof VKontakteTokenStrategy, 'function');
    assert.equal(typeof VKontakteTokenStrategy.Strategy, 'function');
    assert.equal(VKontakteTokenStrategy, VKontakteTokenStrategy.Strategy);
  });

  it('Should properly initialize', function () {
    var strategy = new VKontakteTokenStrategy({
      clientID: '123',
      clientSecret: '123'
    }, function () {
    });
...
describe('VKontakteTokenStrategy:authenticate', function () {
  describe('Authenticate without passReqToCallback', function () {
    var strategy;

    before(function () {
      strategy = new VKontakteTokenStrategy({
        clientID: '123',
        clientSecret: '123'
...
describe('VKontakteTokenStrategy:userProfile', function () {
